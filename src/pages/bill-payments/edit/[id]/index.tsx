...
import { getBillPaymentById, updateBillPaymentById } from 'apiSdk/bill-payments';
...
function BillPaymentEditPage() {
  ...
  const handleSubmit = async (values: BillPaymentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBillPaymentById(id, values);
      mutate(updated);
      resetForm();
